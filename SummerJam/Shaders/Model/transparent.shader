...
/*
//This version will handle multiple layers of transparency, but looks pixelated unless a very high MSAA level is used.
float rand(vec2 co)
{
...
{
	if (rand(ex_texcoords0+float(gl_SampleID))<0.75) discard;
}
*/

void TransparencyFilter()
{
	int ix = int(mod((int(gl_FragCoord.x+0.5)),2)+0.5);
	int iy = int(mod((int(gl_FragCoord.y+0.5)),2)+0.5);
	//if (mod(gl_SampleID,2)==1)
	//{
		if (ix==iy) discard;	
	//}
	//else
